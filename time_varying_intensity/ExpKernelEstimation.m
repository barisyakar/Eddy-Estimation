function [final_lambda0,final_K0,final_omega,final_rho,iter]=ExpKernelEstimation(dataFilePath,init_lambda0,init_K0,init_omega,init_rho,output_workspace_file,num_time_bins,window_steps)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   lambda0_b = sum(Pr0 in bin b) / (GridArea * bin_duration)
...
if nargin < 7 || isempty(num_time_bins); num_time_bins = 28; end
if nargin < 8; window_steps = []; end
if ~isscalar(num_time_bins) || ~isfinite(num_time_bins) || num_time_bins < 1 || fix(num_time_bins) ~= num_time_bins
    error('ExpKernelEstimation:invalidBins', 'num_time_bins must be a positive integer scalar.');
end

% ---- Validate initial parameters ----------------------------------------
% Reject nonfinite values and an all-zero background before the EM iteration.
if isscalar(init_lambda0)
    init_lambda0 = repmat(init_lambda0, num_time_bins, 1);
else
    init_lambda0 = init_lambda0(:);
    if numel(init_lambda0) ~= num_time_bins
        error('ExpKernelEstimation:invalidInit', ...
              'init_lambda0 must be scalar or have exactly num_time_bins=%d entries.', num_time_bins);
    end
end
if any(~isfinite(init_lambda0)) || any(init_lambda0 < 0) || ~any(init_lambda0 > 0)
    error('ExpKernelEstimation:invalidInit', ...
          'init_lambda0 must be finite, nonnegative, and contain at least one positive bin.');
end

params_check = [init_K0, init_omega, init_rho];
params_names = {'init_K0', 'init_omega', 'init_rho'};
for pi = 1:3
    v = params_check(pi);
    if ~isfinite(v) || v <= 0
        error('ExpKernelEstimation:invalidInit', ...
...
lamzero = zeros(n, 1);
ll = zeros(n, 1);
bin_indices = zeros(n, 1);

% Assign every observed event to a piecewise-constant background bin. The
% first timestamp has no predecessors, but it still contributes to lambda0.
event_idx = 0;
for k = 1:K
    ti = vortexarray(k,1).time;
    b_idx = floor(ti / bin_duration) + 1;
    b_idx = min(max(b_idx, 1), num_time_bins);
    for m = 1:numberofvortices(k)
        event_idx = event_idx + 1;
        bin_indices(event_idx) = b_idx;
    end
end
...
initial_params = [mean(newlambda0), newK0, newomega, newrho];
...
for k = 2:K
    for m = 1:numberofvortices(k)
        ...
        lamzero(i) = newlambda0(bin_indices(i));
...
fprintf('Initial: lambda0_mean=%.4e, K0=%.4e, omega=%.4f, rho=%.6f\n\n', ...
    mean(newlambda0), newK0, newomega, newrho);
...
while (params_tolerance > tolerance_threshold) && (iter < max_iter)
    ...
    % lambda0: MLE per bin
    newlambda0 = zeros(num_time_bins, 1);
    for b = 1:num_time_bins
        idx_b = (bin_indices == b);
        newlambda0(b) = sum(Pr0(idx_b)) / (GridArea * bin_duration);
    end
    ...
    fprintf('Iter %d: lambda0_mean=%.4e, K0=%.4e, omega=%.4f, rho=%.6f\n', ...
        iter, mean(newlambda0), newK0, newomega, newrho);
    ...
    %% E-STEP
    newparameters = [mean(newlambda0), newK0, newomega, newrho];
...
fprintf('lambda0_mean=%.6e, K0=%.6e, omega=%.6f, rho=%.6f\n', ...
    mean(final_lambda0), final_K0, final_omega, final_rho);
