...
%
%     Q(t) = lambda0(t)*|S| + K0*E[|a|/b]*H_bar_S
...
Q = zeros(M, 1);
Q(1) = lambda0(1) * GridArea;  % Q(0) = lambda0(0) * |S| (no offspring yet)
...
for n = 2:M
    ...
    t_n = t_grid(n);
    b_idx = floor(t_n / bin_duration) + 1;
    b_idx = min(max(b_idx, 1), num_time_bins);
    forcing = lambda0(b_idx) * GridArea;
    Q(n) = (forcing + dt_actual * history) / denominator;
end
...
if verbose
    ...
    fprintf('  Steady state Q_inf = lambda0_mean*|S|/(1-n) = %.2f\n', ...
            mean(lambda0) * GridArea / max(1 - n_branch, 1e-9));
