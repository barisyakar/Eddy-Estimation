...
%       N_bg,b ~ Poisson(lambda0_b * |S| * bin_duration)
%       Times are uniform within each bin. Locations follow the supplied
%       empirical spatial density, with a uniform distribution as fallback.
%
...
%     lambda0       – scalar or per-bin background rate per unit space/time
...
%
%   Example (lambda0Bins contains the fitted per-area baseline in each bin):
%     cat = simulation('data/Lifetime-first14days.txt', ...
%                       lambda0Bins, 1.49057, 0.778922, 0.0225298, 1344, ...
%                       'SpatialBgMap', bgMap);
%
...
addRequired(p, 'lambda0',     @(v) isnumeric(v) && isvector(v) && ...
                                      all(isfinite(v(:))) && all(v(:) >= 0) && any(v(:) > 0));
...
num_time_bins = length(lambda0);
bin_duration = T / num_time_bins;

N_bg_bins = zeros(num_time_bins, 1);
for b = 1:num_time_bins
    expectedBg = lambda0(b) * GridArea * bin_duration;
    N_bg_bins(b) = poissrnd(expectedBg);
end
N_bg = sum(N_bg_bins);

if verbose
    fprintf('[simulation] Background: E[N_bg_total] = %.1f, sampled N_bg = %d\n', sum(lambda0)*GridArea*bin_duration, N_bg);
end
...
if N_bg > 0
    ...
    for b = 1:num_time_bins
        if N_bg_bins(b) > 0
            idx_end = idx_start + N_bg_bins(b) - 1;
            t_start = (b - 1) * bin_duration;
            allTime(idx_start:idx_end) = t_start + rand(N_bg_bins(b), 1) * bin_duration;
            idx_start = idx_end + 1;
        end
    end
    if useSpatialBg
...
if verbose
    fprintf('\n=== SIMULATION SUMMARY ===\n');
    fprintf('  Parameters: lambda0_mean=%.4e, K0=%.4e, omega=%.4f, rho=%.6f\n', ...
            mean(lambda0), K0, omega, rho);
